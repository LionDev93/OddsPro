...
class OddCard extends React.Component {
  ...
  dropToList1 = e => {
    ...
    this.setState({
      ...
      betType: {
        ...
        list2: s.betType.list2
          .filter(item => item.id != e.id)
          .sort((a, b) => a.id - b.id),
        list3: s.betType.list3
          .filter(item => item.id != e.id)
          .sort((a, b) => a.id - b.id)
      }
    });
  ...
  onUpdRaceNum = async () => {
    ...
    this.setState({
      ...
      betType: {
        ...this.state.betType,
        list1: this.props.cardInfo[this.state.racenum].horseViewList.map(
          item => {
            return {
              id: item.runnerno,
              name: item.horsenamecht
  ...
  render() {
    ...
    let oddList = betList.map((item, i) => {
      ...
      let odd =
        OddsResult[type] && OddsResult[type].find(o => o.pattern == text);
    ...
    return (
      <Grid style={{ paddingBottom: 90 }}>
        <Row style={styles.headerBg}>
          <Col style={styles.headerLeft}>
            <TouchableOpacity
              ...
              onPress={() =>
                this.state.racenum > 1
                  ? this.setState({ racenum: this.state.racenum - 1 }, () =>
                      this.onUpdRaceNum()
                    )
                  : ""
              }
          ...
          <Col style={styles.headerRight}>
            <TouchableOpacity
              ...
              onPress={() =>
                this.state.racenum < this.props.cards.length
                  ? this.setState({ racenum: this.state.racenum + 1 }, () =>
                      this.onUpdRaceNum()
                    )
                  : ""
        ...
        {betList.map((item, i) => {
          const text = item.map(i => ("00" + i).slice(-2)).join("-");
          const odd1 =
            OddsResult[type1] && OddsResult[type1].find(o => o.pattern == text);
          const odd2 =
            OddsResult[type2] && OddsResult[type2].find(o => o.pattern == text);
          return (
            <Row style={styles.horsesBg} key={i}>
              <Col style={styles.row6col1}>
        ...
        {betList.map((item, i) => {
          ...
          return (
            <Row style={styles.horsesBg}>
              ...
              <Col>
                <Item regular style={{ borderColor: "black", marginLeft: 2 }}>
                  <Picker
                    mode="dropdown"
                    iosHeader=""
              ...
              <Col>
                <Text style={styles.row10Text}>
                  {odd ? (odd.odd * amountList[i]).toFixed(2) : "-"}
                </Text>
              </Col>
            </Row>
          );
